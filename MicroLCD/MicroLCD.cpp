...
void LCD_SSD1306::printFloat(float value, int8_t padding)
{
    uint8_t i_part = value;
    uint16_t d_part = (float)((value - (float)i_part) * 10.0);

    printLong(i_part, padding);
    write('.');
    printInt(d_part);
}
...
void LCD_SSD1306::stream(uint8_t c_ascii, uint8_t *data, uint8_t len) {
    Wire.beginTransmission(_i2caddr);
    Wire.write(0x40);
    if (c_ascii > 0x20 && c_ascii < 0x7f) {
        c_ascii -= 0x21;
        for (byte i = 0; i < len; i++) {
            Wire.write(pgm_read_byte(&data+i+(len*c_ascii)));
        }
        Wire.write(0);
    } else
